...
(function () {
  ...
  function renderBudgetSankey(model, isDark) {
    ...
    // Cap outflows au budget, ordre : catégories normales puis épargne/reste (kind)
    ...
    // ── Géométrie ──
    var barW = 26, gapCol = 20;
    var H_BASE = 320, TOP_PAD = 34, BOT_PAD = 22, dy = TOP_PAD;
    var hasGap = specialOut.length > 0 && normalOut.length > 0;
    var H = H_BASE + (hasGap ? gapCol : 0);

    // Colonnes (gauche -> droite)
    var LPz = 150;                 // zone étiquettes revenus (gauche)
    var RPz = 150;                 // zone étiquettes dépenses feuilles (droite)
    var nIncomeCols = incomeHasChildren ? 2 : 1;
    var nExpenseCols = expenseHasChildren ? 2 : 1;
    var nMid = nIncomeCols + 1 + nExpenseCols;   // colonnes de barres
    var innerGap = 150;            // espace entre colonnes (rubans + pastilles catégorie)
    ...
    var xIncGroup = xs[idx++];         // sources (ou income leaves si 1 niveau)
    var xBudget = xs[idx++];
    var xExpGroup = xs[idx++];         // catégories (ou dépenses leaves si 1 niveau)
    ...
    // Étiquettes déportées avec anti-chevauchement + trait de repère
    function labels(nodes, edgeX, anchorX, side) {
      var minGap = 19, top = dy + 2, bot = dy + H - 9;
    ...
    // ── Positions : budget (référence) ──
    // Income sources sur [dy, dy+H_BASE]
    var y = dy, incGroupPos = [];
    income.forEach(function (it) { var h = it.val / totalInc * H_BASE; incGroupPos.push({ it: it, y0: y, y1: y + h }); y += h; });
    // Outflows sur [dy, dy+H (avec gap avant special)]
    y = dy; var outGroupPos = [];
    normalOut.forEach(function (it) { var h = it.val / totalInc * H_BASE; outGroupPos.push({ it: it, y0: y, y1: y + h }); y += h; });
    if (hasGap) y += gapCol;
    specialOut.forEach(function (it) { var h = it.val / totalInc * H_BASE; outGroupPos.push({ it: it, y0: y, y1: y + h }); y += h; });

    // ── Rubans income leaf -> group (si 2 niveaux) ──
    if (incomeHasChildren) {
      incGroupPos.forEach(function (g) {
        var cy = g.y0;
        var kids = g.it.children || [{ name: g.it.name, val: g.it.val, color: g.it.color }];
    ...
    // ── Rubans income group -> budget ──
    incGroupPos.forEach(function (g) { html += ribbon(xIncGroup + barW, g.y0, g.y1, xBudget, g.y0, g.y1, g.it.color, 0.12); });
    // ── Rubans budget -> outflow group (direct vers la feuille si pas d'enfants) ──
    var br = dy;
    ...
    // ── Rubans outflow group -> leaf (si enfants) ──
    ...
    if (incomeHasChildren) {
      incGroupPos.forEach(function (g) {
        ...
        kids.forEach(function (c) { var h = c.val / g.it.val * (g.y1 - g.y0); html += bar(xIncLeaf, cy, h, c.color); cy += h; });
      });
    }
    ...
    if (expenseHasChildren) {
      ...
      html += labels(catNodes, xExpGroup + barW, xExpGroup + barW + 16, 'right');
      var leafNodes = [];
      outGroupPos.forEach(function (g) {
      ...
      html += labels(leafNodes, xExpLeaf + barW, xExpLeaf + barW + 16, 'right');
    } else {
      var expNodes = outGroupPos.map(function (g) { return { yc: (g.y0 + g.y1) / 2, name: g.it.name, val: g.it.val, color: g.it.dot || g.it.color }; });
      html += labels(expNodes, xExpGroup + barW, xExpGroup + barW + 16, 'right');
    }
